...
CHANGELOG v2.0:
  - Fixed: auth_middleware crash (SystemExit → ApplicationHandlerStop)
  - Fixed: admin_only decorator for callback queries
  - Fixed: All admin panel buttons now fully wired
  - Added: @PROBIX_XD auto-granted admin on /start
  - Added: Multi-admin support (DB + secondary usernames)
  - Added: /help, /signal, /stats, /listusers, /removeuser, /setrole, /ping
  - Added: Signal cooldown per symbol (no duplicate spam)
  - Added: Working Settings panel (toggle LSTM, XGBoost, threshold, symbols)
  - Added: Admin panel fully interactive (add/remove/list/broadcast/shutdown)
  - Added: /broadcast supports rich markdown
  - Added: Per-user signal history in /stats
  - Added: Signal strength meter (visual bar)
  - Added: Graceful shutdown with task cleanup
"""
...
async def auth_middleware(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Auto-promote secondary admins the moment they interact
    ...
    if user.id == config.admin_user_id or _is_secondary_admin(username):
        _audit(update, user)
        return  # admins bypass everything
...
class AIEngine:
    ...
    def indicator_scores(self, df: pd.DataFrame) -> Tuple[Dict[str, float], List[str]]:
        ...
        if pd.notna(rsi):
            if rsi < 25:
                scores["rsi"] = 1.0
                reasons.append(f"RSI strongly oversold ({rsi:.1f})")
            elif rsi < 35:
                scores["rsi"] = 0.7
                reasons.append(f"RSI oversold ({rsi:.1f})")
            elif rsi > 75:
                scores["rsi"] = -1.0
                reasons.append(f"RSI strongly overbought ({rsi:.1f})")
            elif rsi > 65:
                scores["rsi"] = -0.7
        ...
        if pd.notna(last.get("stoch_k")) and pd.notna(last.get("stoch_d")):
            if last["stoch_k"] < 20 and last["stoch_d"] < 20:
                scores["stoch"] = 1.0
                reasons.append(f"Stochastic oversold K={last['stoch_k']:.1f}")
            elif last["stoch_k"] > 80 and last["stoch_d"] > 80:
                scores["stoch"] = -1.0
                reasons.append(f"Stochastic overbought K={last['stoch_k']:.1f}")
        ...
        if pd.notna(adx):
            ...
            if adx > 30:
                ...
                reasons.append(f"ADX {adx:.1f} – ranging market")
        ...
        if pd.notna(vol_r):
            if vol_r > 1.5:
                scores["volume"] = 0.8
                reasons.append(f"Volume surge {vol_r:.1f}x avg")
            elif vol_r < 0.5:
                scores["volume"] = -0.3
            else:
                scores["volume"] = 0.0
...
async def signal_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """On-demand signal for a symbol or all symbols."""
    args = context.args
    ...
    for sym in syms:
        ...
        if sig:
            found += 1
            await update.message.reply_text(
                _format_signal(sig), parse_mode=ParseMode.MARKDOWN)
        else:
...
async def add_user_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # We store by username; actual user_id is assigned on their next /start
    await update.message.reply_text(
        f"✅ @{username} whitelisted. They'll get access when they /start the bot."
    )
    # Insert a placeholder so they pass auth immediately on /start
    await db_execute_commit(
...
# ─────────────────────────────────────────
# Conversation state for multi-step admin actions
...
async def admin_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handles free-text input for multi-step admin flows."""
...
# ─────────────────────────────────────────
async def signal_loop(provider: DataProvider, engine: AIEngine, bot_app: Application):
    """Runs in background, broadcasts signals with per-symbol cooldown."""
    ...
    while True:
        try:
            for sym in config.symbols:
                now = time.time()
                # Skip if signal for this symbol was sent recently
                ...
                for uid in users:
                    try:
                        await bot_app.bot.send_message(uid, msg_txt, parse_mode=ParseMode.MARKDOWN)
                        ok += 1
                    except TelegramError:
                        pass
...
async def main():
    ...
    # Persist provider/engine for on-demand /signal
    ...
    # Multi-step admin text input (group=1, after middleware)
    app.add_handler(
